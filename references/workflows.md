...
For converting an existing model into a Modular Avatar-driven setup (regrouping nodes into Clothes/Hair/Accessories, rebuilding the menu tree, moving toggles to MA, fixing FX slider paths), use the dedicated workflow: `references/ma-ify-model.md`.
...
Pitfalls for MA Object Toggle (see `references/gotchas.md`):

- Put `ModularAvatarMenuItem` and `ModularAvatarObjectToggle` on the same GameObject (or ObjectToggle as its child); the parameter comes from the nearest parent MenuItem.
- Set `Objects[].Object.referencePath` to the full avatar-relative path (e.g. `Clothes/Default/Cardigan`). Empty referencePath silently disables the toggle.
- For `_OFF`-style parameters, set `Active = false` (param triggers ⇒ hide).
- Keep menu-tree nodes clean: delete stray MA components on clothing nodes, and avoid duplicate/self-referencing children under the menu tree root.
- MenuInstaller needs `menuToAppend` or a `MenuSource` (e.g. MenuGroup) on the same GameObject.
- Validate with `AvatarProcessor.ProcessAvatarUI()` and check for `MA Responsive: <name>` FX layers.
